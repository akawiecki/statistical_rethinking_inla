...
x[["waic"]][["waic"]]
  ...
  waic.compare <- list( waic= m[["waic"]][["waic"]],
...
test <- lapply(m.list,f.waic.compare) %>%  bind_rows()
...
min.waic <- min(unlist(lapply(m.list, function (x){
x[["waic"]][["waic"]]
} )))

f.waic <- function (x){
list(waic= c(x[["waic"]][["waic"]]),
     fixed= c(paste(x$names.fixed[2:length(x$names.fixed)], collapse= "+")),
     loc.waic= x[["waic"]][["local.waic"]]
     )
}

m.waic.i <-lapply(m.list, f.waic ) #%>% bind_rows()

min.waic.T <- sapply(m.waic.i,function(x) x[["waic"]] == min.waic )

min.waic.list <- m.waic.i[min.waic.T==TRUE ]

d.waic <- lapply(m.waic.i, function (x){x[["waic"]] - min.waic } )


f.waic.diff <- function(x){

 list( d.mean.waic= x[["waic"]]-min.waic.list[[1]][["waic"]] , 
       s.e= as.numeric(sqrt( length(x[["loc.waic"]])*var( x[["loc.waic"]]-min.waic.list[[1]][["loc.waic"]] ) ) ),
      
       #d.loc.waic= x[["loc.waic"]]-min.waic.list[[1]][["loc.waic"]], 
       n= length(x[["loc.waic"]])
 )
...
waic.diff <- lapply(m.waic.i, f.waic.diff) %>% bind_rows()
