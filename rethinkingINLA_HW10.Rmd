...
```{r 10.1 measurement error obs value plot re}
...
```{r 10.1 measurement error est value plot re}
...
d2.i <- d %>% 
  mutate(M = body / max(body, na.rm=TRUE), 
...
#indices of the weights with missing values of body mass
d2.i.na <- which(is.na(d2.i$body))
...
In INLA the models have a different intercept when the analysis is run with the missing data and when it's run only with complete cases!
...
z = m2.2.i$summary.linear.predictor$mean
