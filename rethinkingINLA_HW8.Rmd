...
m1.3.i <- inla(surv ~ 1 + size + f(tank, model="iid", hyper = hcprior), data= d1.i, family = "binomial", 
              Ntrials = density, 
              control.fixed = list(
        mean= 0,
        prec= 0.5, 
...
alist(
S ~ binomial( n , p
logit(p) <- a_bar + z[tank]*sigma + bp[size_]*pred + s[size_], 1 ),
z[tank] ~ normal( 0 bp[size_] ~ normal(
s[size_] ~ normal( 0 , 0.5 ), 
a_bar ~ normal( 0 , 1.5 ), 
sigma ~ exponential( 1 )
),
