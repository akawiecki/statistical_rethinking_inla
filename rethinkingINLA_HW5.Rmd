...
mi.i.df <- bind_cols(est, m1.i.df)

m1.i.plot <- ggplot() + 
    geom_pointrange(aes(x = factor(est), y = mean, ymin = lower, ymax = upper), data= m1.i.df, position = position_dodge(0.5)) +
    xlab("coefficient") +
    ylab("Estimate")+
  coord_flip()+
    theme_bw()
...
I got much worse estimations with INLA 
